...
from telephony.utils import _get_contact_by_phone_number, link_call_with_contact, parse_call_log
...
def set_default_calling_medium(medium):
	if not frappe.db.exists("TP Telephony Agent", frappe.session.user):
		...
		frappe.db.set_value("TP Telephony Agent", frappe.session.user, "default_medium", medium)
...
def get_user_default_calling_medium():
	...
	default_medium = frappe.db.get_value("TP Telephony Agent", frappe.session.user, "default_medium")
